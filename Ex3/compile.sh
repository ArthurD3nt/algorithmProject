...
compile() {
    ...
    # Compile the heap library
    printf "\nCompiling the heap library and it's dependencies ...\n"
    rm -rf ./bin || exit 1
    mkdir bin || exit 1
    cd src || exit 1
    javac heap/Heap.java -d ../bin || exit 1
    javac -classpath ../lib/junit.jar:../lib/hamcrest.jar:. heap/IntegerHeapTestsRunner.java -d ../bin|| exit 1
    cd .. || exit 1
    printf "Done!\n"
}
...
printf "What do you want to do?\n
        1. Just compile the heap library\n
        2. Compile and run the tests for the heap library\n"

read -p "Enter your choice: " choice

case $choice in
1) # Compile the heap library
    compile
    exit 0
    ;;
2) # Compile and run the tests for the heap library
    ...
    cd bin || exit 1
    java -classpath ../lib/junit.jar:../lib/hamcrest.jar:. heap/IntegerHeapTestsRunner || exit 1
    cd ..
    exit 0
    ;;
*)
